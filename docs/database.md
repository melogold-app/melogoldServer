...
| `schema-check.ts` | сверка живой схемы со `schema.snapshot.json` |
...
`read`/`write`/`run` внутри другого `read`/`write`/`run` бросают `NestedDbAccessError` (проверка через `AsyncLocalStorage`, ловит и вызовы глубоко в сервисах). В SQLite одно соединение за мьютексом: вложенный вызов ждал бы внешний вечно.
...
- **Старт сервера** (`prepareDatabase`):
  ...
  5. сверка схемы со снапшотом (таблицы, колонки, физические типы вместе с `COLLATE "C"`, nullable, индексы снапшота, `STRICT`): расхождение при `SCHEMA_CHECK=strict` → выход с кодом 1, при `warn` → предупреждение.
