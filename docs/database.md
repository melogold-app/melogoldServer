...
`db.kysely` (Kysely под `read`/`write`/`run`) — только для инструментов: миграций, сверки схемы, тестов и `scripts/`. ESLint запрещает `.kysely` вне `src/db/**`, тестов и `scripts/`, а чтение `db.kysely` внутри `read`/`write`/`run` бросает `NestedDbAccessError`: в SQLite запрос через него ждал бы единственное соединение вечно, в PostgreSQL выполнился бы вне транзакции.
