...
- **Восстановление доступа.**
  ...
  - Пароль можно сменить с вошедшего устройства. Если устройство сменило пароль без старого, оно попадает под ограничения (§4.8).
- **Упаковка:**
...
| 15 | Восстановление | Код 100 бит (Crockford) плюс смена пароля без старого с устройства старше 7 дней. После неё — кулдаун и ограничения устройства-инициатора | Решение пользователя плюс M6 |
...
### 4.8 Матрица чувствительных действий (M6, m5)
**Определения:**
- `recent(d)`: `d.linked_via ∉ {register, recovery}` и `now < d.created_at + NEW_DEVICE_RESTRICT_HOURS` (24 ч). Правило действует для **любого** нового устройства, в том числе созданного через login.
- **Кулдаун активен:** `users.security_cooldown_until > now`. `started = security_cooldown_started_at`, `initiator = security_cooldown_device_id`.
- `restricted(d)`: кулдаун активен и (`d = initiator` или `d.created_at ≥ started`).
- `protected(t)`: кулдаун активен, `t.created_at < started` и `t ≠ initiator`.

| Действие | Разрешено | Ограничения |
|---|---|---|
| Переименовать своё устройство | всегда | — |
| Переименовать или отозвать чужое устройство `t` | вошедшее устройство | `restricted(me) ∧ protected(t)` → `403 cooldown_restricted`, пароль не помогает. Иначе при `recent(me) ∧ t.created_at < me.created_at` нужен пароль (`403 recent_device_restricted`). **Исключение:** при активном кулдауне устройство без ограничений отзывает `initiator` и устройства, созданные после начала кулдауна, **без пароля** |
| `revoke-others` | то же для каждой цели | Любая `protected` цель при `restricted(me)` → 403. Отзыв без пароля ставит `me.revoked_without_password_at = now` |
| Смена пароля со старым | любое вошедшее | `restricted(me)` → `403 cooldown_restricted` |
| Смена пароля без старого | устройство старше `PASSWORD_RESET_DEVICE_MIN_AGE_DAYS` (7) | Запрещена, если устройство за последние 7 дней отзывало кого-то без пароля, если `restricted(me)` или если активен кулдаун с живым `initiator`. Ответ `403 current_password_required{minDeviceAgeDays}` или `cooldown_restricted`. При успехе: `signOutOtherDevices` принудительно `false`, кулдаун `{until: now+7 д, started: now, initiator: me}`, остальным уходит `account.updated{password_changed_without_old}` |
| Новый код восстановления | пароль | запрещено при активном кулдауне → `403 security_cooldown` |
| Удаление аккаунта | пароль | запрещено при активном кулдауне |
| Одобрение привязки | вошедшее | новое устройство всё равно будет `restricted` |
| Восстановление кодом | код | снимает кулдаун, удаляет все устройства |

**Почему работает:**
- У вора есть разблокированный телефон старше 7 дней.
  - Если он сначала отзовёт все устройства без пароля, сменить пароль без старого 7 дней не сможет. Владелец войдёт по паролю и отзовёт телефон.
  - Если он сначала сменит пароль, начнётся кулдаун. Ни сам телефон, ни новые входы вора с новым паролем не смогут тронуть старые устройства владельца. Любое из них отзывает вора без пароля.
- **Интерфейс:**
  - карточка `account.updated` и постоянная плашка по `/auth/me` (`user.securityCooldown`) с кнопкой «Это не я → отозвать X»;
  - после отзыва устройство владельца может сменить пароль без старого.
...
- **`/auth/recover`**, порядок проверок: политика пароля → код (CAS `WHERE recovery_code_hash=:old`) → argon2 (до транзакции).
  - Итог: новый хеш пароля, `auth_version+1`, кулдаун снят, новый код, **все устройства удалены**, создано новое с `linked_via='recovery'`.
...
| Украденный разблокированный телефон | матрица §4.8, кулдаун, код восстановления |
...
1. **Смена пароля без старого.** Решение «с любого вошедшего устройства» реализовано так:
   - со старым паролем — с любого устройства;
   - без старого — только с устройства старше 7 дней, после чего действует кулдаун и ограничения из §4.8.

   **Рекомендация:** оставить. Иначе украденный разблокированный телефон захватывает аккаунт навсегда. Подтвердите или назовите другой порог.
2. **Лицензия файлов контракта** `openapi/*`, `spec/*` (векторы, `error-codes.json`). Сейчас они под AGPL-3.0, как весь репозиторий.

   **Рекомендация:** CC0-1.0 через `spec/LICENSE` и SPDX-заголовки. Тогда код клиентов, сгенерированный из спецификации, не вызывает лицензионных вопросов у GPL-клиентов и сторонних разработчиков. Нужно ваше согласие как правообладателя.
