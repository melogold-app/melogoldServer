...
| `Hwid` | `^[0-9a-f]{64}$` = `hex(sha256("melogold-hwid-v1\|" + platformId + "\|" + serverId))`. `platformId`: Android `ANDROID_ID`; macOS `IOPlatformUUID`; Windows `MachineGuid\|installSalt`; Linux `/etc/machine-id\|installSalt`; iOS, iPadOS, visionOS — случайный UUID v4 в Keychain с `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly`: переживает переустановку и не переезжает на новое устройство. Запасной для всех — случайный UUID в приватном хранилище. Векторы: `spec/hwid.vectors.json` |
| `Platform` | `^[a-z0-9_]{1,16}$`. Известные значения: `android`, `ios` (iPhone), `ipados` (iPad), `macos`, `visionos` (Vision Pro), `windows`, `linux`, `other`. У Apple Watch своего входа нет: часы работают через iPhone. Хранится как пришло |
